...
- `AlphaConfig` embeds `ServiceConfig` and adds `JobsConfig` (Phase 3 publisher parameters) and `CommanderConfig` (Phase 4 request timeout + history size).
- `BetaConfig` embeds `ServiceConfig` and adds `Zones`, `TelemetryConfig`, `RunnersConfig`, and `ResponderConfig` (Phase 4 ledger size).
