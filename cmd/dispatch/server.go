...
// Server composes all dispatch subsystems.
type Server struct {
	cfg    *config.Config
...
// NewServer creates the dispatch server.
func NewServer(cfg *config.Config) *Server {
	logger := slog.With("service", cfg.Dispatch.Name)
	...
	return &Server{
		...
		info: discovery.ServiceInfo{
			ID:          uuid.New().String(),
			Name:        cfg.Dispatch.Name,
			Endpoint:    fmt.Sprintf("http://%s", cfg.Dispatch.Addr()),
			Health:      "ok",
			Description: cfg.Dispatch.Description,
		},
	}
}
